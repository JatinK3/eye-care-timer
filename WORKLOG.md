...
- [x] **Coach break completion timing**: Coach prompts no longer pause an active break. The break continues through its configured duration with the response retained, then holds at zero until the user selects `Close now`; the close action cannot end a break early.

### Health-Data Integration Discovery (2026-07-21)

**Decision:** The Android integration will be called **Health Connect**, not Google Fit. Google Fit APIs are deprecated and are supported only through the end of 2026; new users should use Health Connect. Health Connect is an on-device, user-permissioned store, not a Google-account or cross-device sync service. BlinkKind's timer history remains its source of truth, and future cloud sync remains a separate feature. See [INTEGRATIONS.md](INTEGRATIONS.md) for the durable architecture and platform context.

- [x] **Discovery complete:** BlinkKind already owns stable timer-event IDs, completed work sessions, daily hydration totals, and JSON backup/export. Hydration currently stores only a daily aggregate, so it cannot be safely backfilled to a health store without inventing timestamps. New health-synced hydration must therefore be recorded as individual local intake events from the point this feature ships.
- [x] **Phase 0 - adapter and data-model spike:** Added the platform-neutral `HealthSyncService`, timestamped hydration events, persisted retryable outbox, and a Kotlin Health Connect bridge using the local event ID/version as the client record identity. Notification-background actions queue local work only; foreground app lifecycle flushes it. Emulator validation covered availability, consent, real write, correction/delete, and the Health Connect access screen.
- [x] **Phase 1 - write-only hydration sync:** Added Android-only `Sync with Health Connect` settings with an explicit disclosure, availability/status, retry, and Manage access controls. It requests only `WRITE_HYDRATION`; each new glass writes its exact volume/timestamp, and undo deletes the matching client record. No history backfill or read permission.
- [ ] **Phase 2 - connected wellness view:** On an explicit second opt-in, request `READ_HYDRATION` and show a clearly attributed, read-only connected total alongside BlinkKind's own total. Preserve data origin and external IDs, never write imported records back, and define duplicate/conflict handling before combining totals. Refresh only on foreground/manual refresh; background health reads are not needed for the product.
- [ ] **Phase 3 - qualified guided-break sync:** Feature-gate Health Connect's mindfulness record support and sync only completed, explicitly selected Box Breathing sessions as mindfulness. Do not misclassify normal eye breaks, focus time, skipped breaks, or generic rest as workouts or medical data. Evaluate eye-exercise/movement mapping only after the hydration path is stable.
- [ ] **Phase 4 - Apple Health parity:** Reuse the same local outbox and service contract for HealthKit hydration, with separate HealthKit entitlement, usage strings, permission UX, and physical-device validation. The Flutter `health` package supports both Health Connect and Apple Health for water, but the adapter decision from Phase 0 governs the implementation.
- [ ] **Release/compliance gate:** Test Android 9-16 availability (Android 14+ system provider and Android 9-13 app provider), permission denial/revocation, offline/retry, undo, duplicate prevention, app restart, and notification-action logging. Settings can already stop sync and manage access.
  - [ ] Publish a public, non-PDF, non-geofenced privacy-policy URL and add it to Play Console. The in-app `Privacy & Health Data` page is available from Settings/About and explains Health Connect, local data, optional services, and access controls; replace its release reminder with the public URL before submission. Explain that BlinkKind writes only user-logged hydration volume/timestamp to on-device Health Connect after opt-in; it does not read, backfill, upload, advertise with, or sell this data. Explain how users stop sync/revoke access and delete local data.
  - [ ] In Play Console: `Policy > App content > Health Apps`, declare **Health and fitness > Nutrition and Weight Management** for hydration tracking. Also declare **Stress Management, Relaxation, Mental Acuity** only if breathing/relaxation guidance ships in the same release. Do not declare a medical device, disease management, or Health Records feature.
  - [ ] Use this Health Connect justification: “BlinkKind lets a user log water glasses. After explicit opt-in, it writes only each newly logged hydration intake, including volume and timestamp, to on-device Health Connect so the user can keep hydration data with compatible wellness apps. BlinkKind does not request read access, backfill old data, upload this data, or use it for advertising.”
  - [ ] In `Policy > App content > Data safety`, audit the complete release—not only Health Connect—including Sentry, analytics, AI providers, and any backend. Health Connect's current write is on-device and has no server transfer; disclose any data transmitted off-device by BlinkKind or its SDKs accurately.
  - [ ] Upload the first Health Connect build to a closed Play test track, submit the declarations, and install from Google Play to validate the package's Health Connect access before production rollout.

**References:** [Google Fit migration guide](https://developer.android.com/health-and-fitness/health-connect/migration/fit), [Health Connect availability](https://developer.android.com/health-and-fitness/health-connect/availability), [Health Connect hydration and mindfulness data types](https://developer.android.com/health-and-fitness/health-connect/data-types), [permission UX guidance](https://developer.android.com/health-and-fitness/health-connect/ui/permissions), and [Google Play health-app declaration](https://developer.android.com/health-and-fitness/health-connect/publish).
...
#### Candidate AI roadmap — pending product selection (2026-07-22)

These are deliberately non-medical, opt-in product ideas. Reuse local timer,
hydration, mood, and break-compliance data where possible; do not add camera
analysis or new health permissions for this roadmap.

- [ ] **AI Focus Planner** — Highest-priority candidate. Let the user enter a task and available time (for example, “finish a presentation in two hours”), generate a preview of realistic focus blocks and breaks, then apply it with one tap. Reuse the existing timer schedule and contextual break-message surfaces.
- [ ] **Weekly Focus Reflection** — Generate a short, explainable weekly summary from local focus time, completed/skipped/postponed breaks, hydration, and optional mood check-ins. Keep it descriptive, non-medical, and fully user initiated.
- [ ] **Contextual Break Guide Selection** — Recommend one existing guide (blink training, breathing, eye exercise, or stretch) based on the current task, optional mood, time of day, and recent break-compliance pattern. Explain why it was suggested and let the user override it.
- [ ] **Natural-language Timer Control** — Convert a narrow set of commands such as “start a 45-minute deep-work session” into a previewed local timer configuration. Never start or change a schedule without an explicit confirmation.
- [ ] **AI Voice-Guided Exercises** — Expand the existing guided eye/breathing exercises with optional speech playback. Generate or select varied scripts, cache them for the active break, and retain a non-AI fallback.
- [ ] **End-of-Session Reflection** — Offer one optional quick question after a work block, then save a concise local productivity note. Never infer health, diagnosis, or emotional state from the response.
- [ ] **Defer camera-based fatigue detection** — Do not implement until after distribution, privacy policy maturity, explicit consent design, and a strong user-benefit case; it introduces sensitive permissions and disproportionate complexity.
...
- [x] **Interactive AI Wellness & Ergonomics Coach** — A chat panel that opens during breaks. The user types a prompt like "My neck hurts" and the AI generates a personalized stretch or breathing routine.
- [x] **AI Focus Mood Tagging** — Mood check-in widget before/after each work session. The AI uses this tag plus break history to personalize context. Persisted alongside `TimerEventRecord`.
...
- [x] **Give the Wellness Coach a failure-safe completion.** Duplicate submissions are ignored and request timeouts present an actionable error. The timer keeps running while a response loads or is shown; only after its natural end does the break hold with an explicit `Close now` action.
...
- [ ] **Apple Health / Health Connect hydration sync**: follow the phased, idempotent health-data plan above; Google Fit is not a valid foundation for new work.
...
- **Wellness Coach completion:** Submitting a typed or suggested question keeps the active desktop break running and preserves the response. If the timer expires while the Coach has been used, the completed break holds until `Close now` is selected, without recording a skip or adding break debt. Coach suggestions rotate through a larger prompt set every 12 seconds and after another question is requested.
...
- Coach loading/error states retain their result through the active countdown, then expose `Close now` only once the Coach-assisted break reaches its natural end. Active-session automatic-pause overrides persist through restart and immediately re-evaluate when disabled.
...
- [x] **Dynamic Fluid Mesh Backgrounds**: A beautifully animated fluid mesh gradient on the home screen that slowly morphs colors based on the current phase (e.g., relaxing cool colors for work, warm/breezy colors for break).
...
- [x] **Glassmorphic Floating Navigation Bar**: A sleek, floating, translucent glassmorphic navigation bar with active-tab micro-animations instead of the standard material navigation.
