...
- [ ] **Phase 0 - adapter and data-model spike:** Add a platform-neutral `HealthSyncService` contract plus persisted, retryable outbox records. Validate the current Flutter health adapter against a real Android device for availability, permission flow, `clientRecordId`/version support, and reliable deletion or correction. Use a small Kotlin Health Connect bridge instead if the package cannot provide idempotent writes. Do not invoke health APIs from the existing notification background isolate; queue the event and flush it when BlinkKind is foregrounded.
- [ ] **Phase 1 - write-only hydration sync:** Add a Settings toggle named `Sync with Health Connect`, availability/install/update guidance, sync status, and a `Manage access` action. Request only `WRITE_HYDRATION`; each newly logged glass becomes one `HydrationRecord` with its exact volume, timestamp, BlinkKind event ID, and manual recording metadata. Corrections/undo must update or delete the matching record rather than write an offsetting duplicate. No historical aggregate backfill and no read permission in this phase.
...
- [ ] **Release/compliance gate:** Test Android 9-16 availability (Android 14+ system provider and Android 9-13 app provider), permission denial/revocation, offline/retry, undo, duplicate prevention, app restart, and notification-action logging. Before Play publication, complete the Health apps declaration, Data safety disclosure, and privacy-policy justification for every requested type. Settings must let users stop syncing and manage access at any time.
