...
- `lib/features/history/history_page.dart`: Range-based daily history, monthly and goal metrics, compliance stats, achievements/milestones, week-over-week trend, recent completed-session detail, export, and confirmed activity clearing.
...
- `lib/services/desktop_integration_service.dart`: Desktop tray, global hotkeys, launch-at-login, start-minimized startup behavior, dynamic tray countdown icons, window lifecycle, and native Linux break-window entry/exit coordination with previous-state restoration.
...
- Settings includes History ranges for 7 days, 30 days, and all active days, plus current-month totals, goal completion rate, best day, compliance rate, achievements, weekly trend, recent completed sessions, export, and confirmed activity clearing.
...
- Desktop tray support includes dynamic countdown/status icons, next-break tooltip text, start/pause/resume/take-break-now/skip/postpone/snooze actions, global hotkeys, launch-at-startup, and an optional start-minimized setting for tray-first login workflows. Home also exposes Take break now and snooze quick actions.
